...
function onVideoEventTwitch(event) {
...
function startRecording() {
    ...
    sendStartMessage();
    registerVideoEvents(htmlplayer, onVideoEventTwitch);
...
function stopRecording() {
    ...
    recording = false;
    try {
        unregisterVideoEvents(onVideoEventTwitch);
    }
    catch(e){
        console.log(e)
    }
...
chrome.runtime.onMessage.addListener(
    function(request, sender, sendResponse) {
        if(request.message === "twitch_page_reloaded") {
            pageChange(document.URL);
        } else if(request.message === "general_page_reloaded") {
            if (!recording && document.URL.includes('twitch.tv') ) {
                console.log("It's a twitch page: " + document.URL);
                recording = true;
                pageLoadTimestamp = (new Date());
                trackVideoTag(5, 1000, 0, htmlplayer, myInterval, startRecording);
            }
